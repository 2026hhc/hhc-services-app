...
// === FORM COMPONENTS (separate state = no re-render of parent) ===
function PatientModal({onSubmit}){
  const [f,setF]=useState({name:"",address:"",pathology:"",doctor:"",mutuelle:""});
  return(<>
    ...
    {[["name","Nom"],["address","Adresse"],["pathology","Pathologie"],["doctor","Médecin"],["mutuelle","Mutuelle"]].map(([k,l])=><div key={k}><Label>{l}</Label><Inp value={f[k]} onChange={e=>setF(p=>({...p,[k]:e.target.value}))}/></div>)}
    <Btn onClick={()=>{if(!f.name)return;onSubmit(f);}}>Ajouter</Btn>
  </>);
}
function ScheduleModal({onSubmit}){
  const [f,setF]=useState({sid:"",d:0,sh:""});
  return(<>
    ...
    <Label>Personnel</Label><Sel value={f.sid} onChange={e=>setF(p=>({...p,sid:e.target.value}))}><option value="">Choisir...</option>{field.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</Sel>
    <Label>Jour</Label><div style={{display:"flex",gap:4,marginBottom:14}}>{DAYS_W.map((d,i)=><Chip key={i} on={f.d===i} onClick={()=>setF(p=>({...p,d:i}))}>{d}</Chip>)}</div>
    <Label>Horaire</Label><Sel value={f.sh} onChange={e=>setF(p=>({...p,sh:e.target.value}))}><option value="">Choisir...</option><option>Matin</option><option>AM</option><option>Nuit</option></Sel>
    <Btn onClick={()=>{if(!f.sid)return;onSubmit(f);}}>Ajouter</Btn>
  </>);
}
function TaskModal({channels,onSubmit}){
  const [f,setF]=useState({ch:"",title:"",desc:"",to:"",pr:"moyenne"});
  return(<>
    ...
    <Label>Canal</Label><Sel value={f.ch} onChange={e=>setF(p=>({...p,ch:e.target.value}))}><option value="">Choisir...</option>{channels.map(ch=><option key={ch.id} value={ch.id}>{ch.name}</option>)}</Sel>
    <Label>Titre</Label><Inp value={f.title} onChange={e=>setF(p=>({...p,title:e.target.value}))}/>
    <Label>Description</Label><Ta value={f.desc} onChange={e=>setF(p=>({...p,desc:e.target.value}))}/>
    <Label>Assigné à</Label><Sel value={f.to} onChange={e=>setF(p=>({...p,to:e.target.value}))}><option value="">Moi-même</option>{STAFF.map(s=><option key={s.id} value={s.id}>{s.photo} {s.name}</option>)}</Sel>
    <Label>Priorité</Label><div style={{display:"flex",gap:8,marginBottom:14}}>{PRIOS.map(p=><span key={p.k} onClick={()=>setF(pr=>({...pr,pr:p.k}))} style={{padding:"6px 14px",borderRadius:12,fontSize:13,fontWeight:600,background:`${p.c}20`,color:p.c,cursor:"pointer",outline:f.pr===p.k?`2px solid ${p.c}`:"none",outlineOffset:2}}>{p.l}</span>)}</div>
    <Btn disabled={!f.title||!f.ch} onClick={()=>onSubmit(f)}>Créer</Btn>
  </>);
}
function LeaveModal({user:u,onSubmit}){
  const [f,setF]=useState({sid:"",type:"",from:"",to2:"",days:"",reason:""});
  return(<>
    ...
    {(u.isRH||u.admin)&&<><Label>Employé</Label><Sel value={f.sid} onChange={e=>setF(p=>({...p,sid:e.target.value}))}><option value="">Choisir...</option>{allNoDir.map(s=><option key={s.id} value={s.id}>{s.photo} {s.name}</option>)}</Sel></>}
    <Label>Type</Label><Sel value={f.type} onChange={e=>setF(p=>({...p,type:e.target.value}))}><option value="">Choisir...</option>{LEAVE_T.map(t=><option key={t}>{t}</option>)}</Sel>
    <Label>Début</Label><Inp type="date" value={f.from} onChange={e=>setF(p=>({...p,from:e.target.value}))}/>
    <Label>Fin</Label><Inp type="date" value={f.to2} onChange={e=>setF(p=>({...p,to2:e.target.value}))}/>
    <Label>Jours</Label><Inp type="number" value={f.days} onChange={e=>setF(p=>({...p,days:e.target.value}))}/>
    <Label>Motif</Label><Ta value={f.reason} onChange={e=>setF(p=>({...p,reason:e.target.value}))}/>
    <Btn onClick={()=>{if(!f.type||!f.from)return;onSubmit(f);}}>Soumettre</Btn>
  </>);
}
function PostModal({onSubmit}){
  const [f,setF]=useState({title:"",body:"",cat:"general"});
  return(<>
    ...
    <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>{POST_CATS.filter(c=>c.k!=="all").map(c=><span key={c.k} onClick={()=>setF(p=>({...p,cat:c.k}))} style={{padding:"6px 14px",borderRadius:12,fontSize:13,fontWeight:600,background:`${c.c}20`,color:c.c,cursor:"pointer",outline:f.cat===c.k?`2px solid ${c.c}`:"none",outlineOffset:2}}>{c.l}</span>)}</div>
    <Label>Titre</Label><Inp placeholder="Ex: Mise à jour protocole..." value={f.title} onChange={e=>setF(p=>({...p,title:e.target.value}))}/>
    <Label>Contenu</Label><Ta style={{minHeight:120}} placeholder="Détails..." value={f.body} onChange={e=>setF(p=>({...p,body:e.target.value}))}/>
    <Btn disabled={!f.title} onClick={()=>onSubmit(f)}>Publier</Btn>
  </>);
}
function RapportForm({patients:pts,user:u,onSubmit}){
  const [f,setF]=useState({pid:"",cares:[],notes:"",dur:"30",ta:"",temp:"",pulse:""});
  return(
    <Card>
      ...
      <Label>Patient</Label>
      <Sel value={f.pid} onChange={e=>setF(p=>({...p,pid:e.target.value}))}><option value="">Sélectionner...</option>{pts.filter(p=>p.status==="actif").map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</Sel>
      <Label>Soins</Label>
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>{CARES.map(c=><Chip key={c} on={f.cares.includes(c)} onClick={()=>setF(p=>({...p,cares:p.cares.includes(c)?p.cares.filter(x=>x!==c):[...p.cares,c]}))}>{c}</Chip>)}</div>
      <Label>Durée</Label>
      <div style={{display:"flex",gap:6,marginBottom:14}}>{["15","25","30","45","60"].map(d=><div key={d} style={{flex:1,padding:"10px 0",textAlign:"center",borderRadius:10,fontSize:14,fontWeight:600,cursor:"pointer",border:`1px solid ${f.dur===d?teal:dark}`,background:f.dur===d?`${teal}20`:"transparent",color:f.dur===d?teal:gray}} onClick={()=>setF(p=>({...p,dur:d}))}>{d}m</div>)}</div>
      <Label>Signes vitaux</Label>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>
        {[["ta","Tension","13/8"],["temp","Temp °C","36.8"],["pulse","Pouls","72"]].map(([k,l,ph])=><div key={k}><Inp placeholder={ph} style={{textAlign:"center",marginBottom:4}} value={f[k]} onChange={e=>setF(p=>({...p,[k]:e.target.value}))}/><div style={{fontSize:11,color:gray,textAlign:"center"}}>{l}</div></div>)}
      </div>
      <Label>Observations</Label>
      <Ta placeholder="État du patient..." value={f.notes} onChange={e=>setF(p=>({...p,notes:e.target.value}))}/>
      <Btn disabled={!f.pid||f.cares.length===0} onClick={()=>{onSubmit(f);setF({pid:"",cares:[],notes:"",dur:"30",ta:"",temp:"",pulse:""});}}>📤 Envoyer le rapport</Btn>
    </Card>
  );
}
function CommentBox({onSubmit}){
  ...
  return(
    <div style={{display:"flex",gap:8,marginTop:8}}>
      <Inp placeholder="Commentaire..." style={{marginBottom:0,flex:1}} value={txt} onChange={e=>setTxt(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&txt.trim()){onSubmit(txt);setTxt("");}}}/>
      <Btn small onClick={()=>{if(!txt.trim())return;onSubmit(txt);setTxt("");}}>Envoyer</Btn>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(()=>{if(tab==="pointage"&&!modal){const t=setInterval(()=>setNow(new Date()),1000);return()=>clearInterval(t);}},[tab,modal]);
  ...
  const actives = useMemo(()=>field.filter(n=>getSt(n.id).s==="on").length,[getSt]);
  const todayR = useMemo(()=>reports.filter(r=>new Date(r.time).toDateString()===now.toDateString()),[reports,now]);
  const pendR = useMemo(()=>reports.filter(r=>!r.valid),[reports]);
  const totMin = useMemo(()=>todayR.reduce((s,r)=>s+r.dur,0),[todayR]);
  const paidI = useMemo(()=>invs.filter(i=>i.st==="payée").reduce((s,i)=>s+i.amt,0),[invs]);
  const pendI = useMemo(()=>invs.filter(i=>i.st==="en attente").reduce((s,i)=>s+i.amt,0),[invs]);
  const myCh = useMemo(()=>user ? CHANNELS.filter(ch=>ch.members.includes(user.id)) : [],[user]);
  const pendL = useMemo(()=>leaves.filter(l=>l.st==="en_attente").length,[leaves]);
  const totPay = useMemo(()=>salaries.reduce((s,p)=>s+p.net,0),[salaries]);
  const paidPay = useMemo(()=>salaries.filter(s=>s.st==="payé").reduce((s,p)=>s+p.net,0),[salaries]);
  const lvBal = useCallback(id=>{const u=leaves.filter(l=>l.sid===id&&l.st==="approuvé"&&l.type==="Congé annuel").reduce((s,l)=>s+l.days,0);return{tot:20,u,r:20-u};},[leaves]);
  ...
  const renderContent = () => {
    ...
    if(tab==="rapport") return (
      <div>
        <RapportForm patients={patients} user={user} onSubmit={f=>{
          setReports(p=>[...p,{id:p.length+1,sid:user.id,pid:parseInt(f.pid),cares:[...f.cares],notes:f.notes,dur:parseInt(f.dur),vitals:{ta:f.ta,temp:f.temp,p:f.pulse},time:new Date().toISOString(),loc:gL(),valid:false}]);
        }}/>
      </div>
    );
      ...
      if(subView?.type==="post"){
        ...
        return(<div style={{}}>
          ...
          <Card>
            ...
            <CommentBox onSubmit={txt=>{setPosts(pp=>pp.map(x=>x.id===p.id?{...x,comments:[...x.comments,{a:user.id,t:txt,time:new Date().toISOString()}]}:x));}}/>
          </Card>
        </div>);
      }
      // Board list
      ...
      if(subView?.type==="task"){
        ...
        return(<div style={{}}>
          ...
          <Card>
            ...
            <CommentBox onSubmit={txt=>{setTasks(p=>p.map(x=>x.id===t.id?{...x,comments:[...x.comments,{a:user.id,t:txt,time:fS(new Date())}]}:x));}}/>
          </Card>
        </div>);
      }
      // Channel view
      if(subView?.type==="channel"){
        ...
        return(<div style={{}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            ...
            <Btn small onClick={()=>setModal("task")}>+ Tâche</Btn>
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
  ...
  return (
    <div style={{fontFamily:"'Outfit',sans-serif",background:bg,minHeight:"100vh",color:light,maxWidth:500,margin:"0 auto"}}>
      ...
      {/* MODALS */}
      {modal&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:500}} onClick={()=>setModal(null)}>
        <div style={{background:"#0f2027",borderRadius:"16px 16px 0 0",padding:20,width:"100%",maxWidth:500,maxHeight:"80vh",overflow:"auto",border:"1px solid rgba(255,255,255,0.1)",WebkitOverflowScrolling:"touch"}} onClick={e=>e.stopPropagation()}>
          <div style={{width:40,height:4,borderRadius:2,background:"#2a3f55",margin:"0 auto 16px"}}/>
          {modal==="patient"&&<PatientModal onSubmit={f=>{setPatients(p=>[...p,{...f,id:p.length+1,status:"actif"}]);setModal(null);}}/>}
          {modal==="schedule"&&<ScheduleModal onSubmit={f=>{setSched(p=>[...p,{sid:parseInt(f.sid),d:f.d,sh:f.sh,pts:[]}]);setModal(null);}}/>}
          {modal==="task"&&<TaskModal channels={myCh} onSubmit={f=>{setTasks(p=>[{id:p.length+1,ch:f.ch,title:f.title,desc:f.desc,auth:user.id,to:parseInt(f.to)||user.id,pr:f.pr,st:"a_faire",comments:[]},...p]);setModal(null);}}/>}
          {modal==="leave"&&<LeaveModal user={user} onSubmit={f=>{const sid=(user.isRH||user.admin)?parseInt(f.sid)||user.id:user.id;setLeaves(p=>[...p,{id:p.length+1,sid,type:f.type,from:f.from,to:f.to2||f.from,days:parseInt(f.days)||1,reason:f.reason,st:"en_attente",by:null}]);setModal(null);}}/>}
          {modal==="post"&&<PostModal onSubmit={f=>{setPosts(p=>[{id:p.length+1,auth:user.id,title:f.title,body:f.body,cat:f.cat,pin:false,time:new Date().toISOString(),comments:[]},...p]);setModal(null);}}/>}
        </div>
      </div>}
